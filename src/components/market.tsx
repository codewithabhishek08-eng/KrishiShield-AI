...
import { Search, Star, ArrowLeft, TrendingUp, TrendingDown, Clock, ShieldCheck, Info, Sparkles } from 'lucide-react';
...
export function MarketScreen() {
  ...
  return (
    <div className="flex flex-col gap-4 animate-in pb-20">
      <div className="px-1">
        <h2 className="text-2xl font-headline font-black text-white">Market Intelligence — {profile.city}, {profile.state}</h2>
        <Badge variant="outline" className="bg-primary/10 text-primary border-none mt-1">{profile.state} Regional Supply Update</Badge>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 h-[calc(100vh-220px)] overflow-hidden">
        ...
        <div className={cn("lg:w-[380px] w-full flex flex-col gap-4 bg-white/[0.03] border border-white/10 rounded-2xl p-4 overflow-hidden", mobileView === 'detail' ? 'hidden lg:flex' : 'flex')}>
          <div className="relative">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search 200 crops..." className="bg-white/5 border-white/10 pl-9 rounded-xl" />
          </div>
            ...
            {filteredCrops.map(c => (
              <div key={c.id} onClick={() => { setSelectedCropId(c.id); setMobileView('detail'); }} className={cn("flex items-center gap-4 px-4 py-3 cursor-pointer transition-all hover:bg-white/5", selectedCropId === c.id ? "bg-primary/5 border-l-4 border-primary" : "border-l-4 border-transparent")}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] opacity-30 uppercase">Price in {profile.city} Mandi</p>
                </div>
        ...
        <div className={cn("flex-1 bg-white/[0.03] border border-white/10 rounded-2xl flex flex-col overflow-hidden", mobileView === 'list' ? 'hidden lg:flex' : 'flex')}>
          <div className="p-6 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs opacity-40">Local Analysis for {profile.city} Mandi</p>
              </div>
            </div>
            <Button size="sm" className="bg-primary hover:bg-primary/90 text-[11px] font-black h-8">Lock Price in {profile.city}</Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
             <div className="bg-primary/5 border border-primary/20 p-6 rounded-2xl space-y-4">
               ...
               {loadingForecast ? <div className="space-y-2 animate-pulse"><div className="h-4 bg-white/5 w-full rounded" /><div className="h-4 bg-white/5 w-3/4 rounded" /></div> : 
                 <p className="text-[15px] italic text-white/80 leading-relaxed">&quot;{priceForecast}&quot;</p>}
             </div>
             {/* Additional market UI blocks */}
          </div>
        </div>
      </div>
    </div>
