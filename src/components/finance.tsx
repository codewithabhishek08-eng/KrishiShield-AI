...
import { Landmark, CheckCircle2, TrendingUp, ShieldCheck, UserCheck, Quote, ChevronDown, ChevronUp } from 'lucide-react';
...
interface EligibilityExplanation {
  summary: string;
  factors: { label: string; impact: 'positive' | 'neutral'; explanation: string }[];
}
...
export function FinanceScreen() {
  ...
  useEffect(() => {
    ...
    const loadIntelligentContent = async () => {
      // Impact Story
      try {
    ...
    <div className="space-y-16">
      <section className="animate-in">
        <div className="max-w-4xl mx-auto bg-card border border-white/5 rounded-[2.5rem] p-8 md:p-12 overflow-hidden relative">
          <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none"><Landmark size={240} /></div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 relative z-10">
            <div className="space-y-8">
              <h3 className="text-3xl font-headline font-black leading-tight">Harvest-Backed <br /> Instant Liquidity</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-3 bg-background/40 rounded-xl border border-white/5">
                    ...
                    {checks[i] ? <CheckCircle2 size={16} className="text-primary animate-in scale-110" /> : <div className="w-4 h-4 rounded-full border border-white/10 border-t-primary animate-spin" />}
                  </div>
                ))}
              </div>
              ...
              {checks.every(c => c) && (
                <div className="animate-in space-y-6">
                  <div className="p-6 bg-primary/5 border border-primary/20 rounded-2xl relative">
                  ...
                  <Collapsible open={isOpen} onOpenChange={setIsOpen} className="w-full">
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" className="w-full flex justify-between text-xs font-bold uppercase tracking-wider h-8">
                        Why am I eligible?
                        {isOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                      </Button>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="pt-4 space-y-4">
                      {loadingExplanation ? <Skeleton className="h-20 w-full" /> : explanation && (
                            ...
                            {explanation.factors && explanation.factors.length > 0 ? explanation.factors.map((f, i) => (
                              <div key={i} className="flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                  <span className="text-[10px] font-bold uppercase block">{f.label}</span>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-8">
                {[{ label: 'KrishiShield AI', rate: 1, color: 'bg-primary' }, { label: 'Local Moneylender', rate: 36, color: 'bg-muted' }, { label: 'Traditional Bank', rate: 15, color: 'bg-muted' }].map((item, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex justify-between text-xs font-bold uppercase tracking-wider"><span>{item.label}</span><span className={item.rate === 1 ? 'text-primary' : ''}>{item.rate}% p.a.</span></div>
                    <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                      <div className={`h-full ${item.color} transition-all duration-1000 ease-out delay-500`} style={{ width: `${(item.rate / 36) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-12 p-6 bg-[#0F1123] rounded-3xl border border-white/5">
                <p className="text-sm opacity-60 leading-relaxed italic">"Traditional lenders rely on credit scores. We rely on your soil health and satellite proof. That's why we can offer the lowest rates in the industry."</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="animate-in space-y-12">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {[{ label: 'Income Increase', val: 22, s: '%', p: '+' }, { label: 'Risk Reduction', val: 90, s: '%', p: '' }, { label: 'Capital Saved', val: 2.4, s: 'L', p: '₹' }, { label: 'Farmers Protected', val: 47, s: '', p: '' }].map((m, i) => (
            <div key={i} className="bg-card border border-white/5 p-6 rounded-3xl text-center group hover:border-primary/30 transition-all">
              <p className="text-[40px] font-headline font-black mb-1 group-hover:scale-110 transition-transform block"><Counter target={m.val} prefix={m.p} suffix={m.s} /></p>
        ...
        <div className="max-w-3xl mx-auto relative">
          {loadingStory ? <Skeleton className="h-32 w-full rounded-3xl" /> : (
            <div className="relative p-12 bg-card border border-white/5 rounded-[3rem] text-center overflow-hidden">
              <Quote size={48} className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 text-primary opacity-20" />
              <p className="text-xl md:text-2xl font-light italic leading-relaxed opacity-90 mb-8">"{impactStory}"</p>
              <div className="flex flex-col items-center gap-3">
                <div className="w-12 h-12 rounded-full ring-2 ring-primary/20 ring-offset-4 ring-offset-background bg-primary/40" />
                <div><p className="font-bold text-sm">Ramesh Kumar</p><p className="text-[10px] uppercase font-black tracking-widest opacity-30">Tomato Farmer • Nasik</p></div>
              </div>
            </div>
          )}
        </div>
